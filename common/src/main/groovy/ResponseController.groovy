...
class ResponseController {
    ...
    static DocumentInfo getDocument(List<FxResponse> responses){
        ...
        for (Map.Entry<String, List<FxResponse>> symbolKey in responseMap) {
            ...
            fillSymbolRequestInfo(symbolInfo, symbolKey.value)
            symbolRequestList.add(symbolInfo)
    ...
    static SymbolRequestInfo fillSymbolRequestInfo(SymbolRequestInfo toFill, List<FxResponse> resp) {
        ...
        for (element in resp) {
            ...
            sum.add(new BigDecimal(element.amount))
        }

        toFill.average = sum.divide(BigDecimal.valueOf(
                resp.size()).setScale(2, BigDecimal.ROUND_HALF_UP)).toString()
        toFill.max = max
        toFill.min = min
...
//    old version
//    String getMinAmount(List<FxResponse> res) {
//        res.stream()
//            .map({ resp -> resp.amount })
//            .min({ a1, a2 -> new BigDecimal(a1) <=> new BigDecimal(a2)})
//    }
//
//    String getMaxAmount(List<FxResponse> res) {
//        res.stream()
//                .map({ resp -> resp.amount })
//                .max({ a1, a2 -> new BigDecimal(a1) <=> new BigDecimal(a2)})
//    }
//
//    String getAverageAmount(List<FxResponse> res) {
//        BigDecimal sum = BigDecimal.ZERO
//        for(response in res) {
//            sum.add(new BigDecimal(response.amount))
//        }
//        sum.divide(BigDecimal.valueOf(res.size()).setScale(2, BigDecimal.ROUND_HALF_UP)).toString()
//    }
}
